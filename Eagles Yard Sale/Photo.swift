...
//        let date = dictionary["date"] as! Date? ?? Date()
    ...
    func saveData(post: Post, completed: @escaping (Bool) -> ()) {
        ...
        let storageRef = storage.reference().child(post.documentID).child(self.documentUUID)
        ...
        uploadTask.observe(.success) { (snapshot) in
            ...
            let ref = db.collection("posts").document(post.documentID).collection("photos").document(self.documentUUID)
            ref.setData(dataToSave) { (error) in
                if let error = error {
                    print("*** ERROR: updating document \(self.documentUUID) in spot \(post.documentID) \(error.localizedDescription)")
                    completed(false)
                } else {
        ...
        uploadTask.observe(.failure) { (snapshot) in
            if let error = snapshot.error {
                print("*** ERROR: upload task for file \(self.documentUUID) failed, in spot \(post.documentID), error \(error)")
            }
            return completed(false)
        }
    }
